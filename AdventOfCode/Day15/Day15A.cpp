...
void Day15A::ProcessData()
{
    output = DiscoverNeighbourhood(programCode);
}

void Day15A::SaveOutput()
{
    std::cout << "Result: " << output << std::endl;
}

long long Day15A::RunProgram(std::vector<long long>& programCode)
{
...
int Day15A::DiscoverNeighbourhood(std::vector<long long>& programCode, int x, int y, int distance)
{
    ...
    if (visitedPoints.find(currentPoint) == visitedPoints.end())
    {
        ...
        for (int direction = 1; direction <= 4; ++direction)
        {
            ...
            if (1ll == output)
            {
                ...
                distanceToOxygenSystem += DiscoverNeighbourhood(
                    programCodeCopy,
